...
\begin{block}{Research Objective}
  ...
  stages increasingly important. Autonomous recovery vehicles (ARVs) address this by
  reducing cost, human exposure, and environmental footprint. This requires solving
  two coupled problems. The rocket's landing location must be predicted in real time
  so the ARV can be guided to that location. 
...
\begin{block}{Andøya Test Flight}
  ...
  \begin{minipage}[t]{0.58\columnwidth}
    \vspace{0pt}
    Andøya student-rocket flight (Mongoose~98), 90\,s flight to
    impact. The map shows the rocket ground track (red) and the IPP
    predictions coloured by flight time, with purple indicating early
    predictions and yellow indicating late predictions converging toward impact.

    \vspace{0.5em}
    The onboard sensor suite comprised an IMU, GNSS, and
    magnetometer. Barometer data was not available on this flight.
    Wind profiles were provided by a weather balloon launched
    30--45\,min before launch. The rocket impacted approximately
    6.3\,km from the launch pad.

    \vspace{0.5em}
    Outlier predictions during the burn phase stem from initial yaw uncertainty
    and attitude instability caused by gyroscope saturation under high spin,
    possibly compounded by thrust model errors. Once ballistic,
    predictions converge toward the actual impact, marked by the crosshair.
  \end{minipage}
\end{block}
...
\begin{block}{IPP Prediction Error}
  \begin{center}
    \includegraphics[width=0.97\columnwidth]{figures/poster_ipp_error.pdf}
  \end{center}
  \vspace{0.3em}
  During the burn phase ($t \lesssim 10$\,s) the prediction is corrupted by
  gyroscope and accelerometer saturation under high spin, vibration, GNSS error,
  and thrust/mass mismodelling. After burnout the error drops sharply and decays
  through to impact. Errors are reported against the GNSS reference.
\end{block}
